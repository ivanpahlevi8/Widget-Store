...
func (app *application) SendEmail(to, from, subject, tmpl string, data interface{}) error {
...
func RenderHtmlTemplate(temp string, data interface{}) (string, error) {
	...
	pathToTemplate := fmt.Sprintf("templates/%s.html.tmpl", temp)
...
func RenderPlainTemplate(temp string, data interface{}) (string, error) {
	// create path to template
	pathToTemplate := fmt.Sprintf("templates/%s.plain.tmpl", temp)
